# Count Placements for 2014, by 'level of english required' and "nationality" in df_oppsaltm
...
vols_placed_by_eng_level_nationality <- placements_vols_opps %>%
    filter(Placement_Date_Year == "2014") %>%
    group_by(Level.of.English.required.for.role, Nationality) %>%
