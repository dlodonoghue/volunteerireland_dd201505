...
# year, segmented by volunteer age group
...
# total volunteers placed by Age.Group and Year ----------------------------------
vols_placed_by_age_group    <- placements_and_vols %>%
                    group_by(Placement_Date_Year, Age.Group) %>%
                    summarise(Num_vols_placed = n_distinct(Volunteer_ID)) %>%
                    ungroup()                     

View(vols_placed_by_age_group)

# sanity check; is total volunteers placed in 2014 equal to 4555? -------------
df <- vols_placed_by_age_group[vols_placed_by_age_group$Placement_Date_Year == "2014",]
