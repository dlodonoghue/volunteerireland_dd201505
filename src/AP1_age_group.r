...
# in each calendar year, segmented by volunteer age group
...
vols_applied_by_age_group   <- applications_and_vols %>%
    group_by(Application_Date_Year, Age.Group) %>%
    summarise(Num_vols_applied = n_distinct(Volunteer_ID)) %>%
    ungroup()                     

View(vols_applied_by_age_group)
