# Count Placements for 2014, by 'publish_direct_contact' in df_oppsaltm,
# where Vol req'd <21 in Opp record
...
# add opportunities to get the number of volunteers required and Publish.Direct.Contact.
data <- merge(x = placements_and_vols, y = opportunities[, c("Opportunity_ID",
                                                             "Number.of.Volunteers.Required",
                                                             "Publish.Direct.Contact.")],
              by = "Opportunity_ID", all.x = TRUE)
rm(placements_and_vols)

vols_placed_by_PDC <- data %>%
  filter(Placement_Date_Year == "2014" &
           Number.of.Volunteers.Required < 21) %>%
  group_by(Placement_Date_Year, Publish.Direct.Contact.) %>%
...
View(vols_placed_by_PDC)

rm(data)
rm(vols_placed_by_PDC)
